...
export default class ChatClientRelay {
  ...
  async wsConnect() {
    ...
    let wsBaseUrl = baseUrl.replace(/^http(s?):/, 'ws$1:')
    let wsUrl = new URL('/api/chat', wsBaseUrl)
    // 透传 overlay URL 上的 token，兼容服务端 `?token=...` 鉴权。
    let pageToken = new URL(window.location.href).searchParams.get('token')
    if (pageToken) {
