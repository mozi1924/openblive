...
export function getShowGuardLevelText(guardLevel: number) {
...
export function getPriceConfig(price: number) {
  let i = 0;
...
export function getShowContent(message: { content: string; translation?: string }) {
...
export function getShowContentParts(message: { isMirror?: boolean; contentParts: any[]; translation?: string }) {
  const frontParts: any[] = [];
  if (message.isMirror) {
    frontParts.push({
      type: CONTENT_PART_TYPE_TEXT,
      text: i18n.t("chat.mirrorMsg"),
    });
  }
  const backParts: any[] = [];
  if (message.translation) {
...
export function getGiftShowContent(message: { giftName: string; num: number }, showGiftName: boolean) {
  if (!showGiftName) {
    return "";
  }
  return i18n.t("chat.sendGift", { giftName: message.giftName, num: message.num });
}

export function getGiftShowNameAndNum(message: { giftName: string; num: number }) {
  return `${message.giftName}x${message.num}`;
}

export function getShowAuthorName(message: { authorName: string; authorNamePronunciation?: string }) {
