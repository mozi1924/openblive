...
import "../../assets/css/youtube/yt-live-chat-author-badge-renderer.css";
import "../../assets/css/youtube/yt-icon.css";
...
export default function AuthorBadge({ isAdmin, privilegeType, className }: AuthorBadgeProps) {
  const authorTypeText = isAdmin ? "moderator" : privilegeType > 0 ? "member" : "";
  const readableAuthorTypeText = isAdmin
    ? i18n.t("chat.moderator")
    : constants.getShowGuardLevelText(privilegeType);

  const classes = className || "";

  return (
    <yt-live-chat-author-badge-renderer type={authorTypeText} class={classes} className={classes}>
      <el-tooltip content={readableAuthorTypeText} placement="top">
        <div id="image" className="style-scope yt-live-chat-author-badge-renderer">
          {isAdmin ? (
            ...
            <img
              src={`${import.meta.env.BASE_URL}static/img/icons/guard-level-${privilegeType}.png`}
